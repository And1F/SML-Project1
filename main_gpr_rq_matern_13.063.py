...
if __name__ == "__main__":
    ...
    # Combined kernel: RationalQuadratic + Matern
    ...
    param_dist = {
        ...
        'regressor__kernel__k2__k1__length_scale': uniform(0.1, 2.0),  # RationalQuadratic part
        'regressor__kernel__k2__k1__alpha': uniform(0.1, 5.0),         # RationalQuadratic alpha
        'regressor__kernel__k2__k2__length_scale': uniform(0.1, 2.0),  # Matern part
    }
    ...
    # Save best pipeline
    joblib.dump(random_search.best_estimator_, config["data_dir"] / "pipeline_random_rq_matern.joblib")
    print("[INFO]: Best pipeline saved to data/pipeline_random_rq_matern.joblib")

    # Save RandomizedSearchCV results to CSV
    results_df = pd.DataFrame(random_search.cv_results_)
    results_df.to_csv(config["data_dir"] / "randomsearch_results_rq_matern.csv", index=False)
    print("[INFO]: RandomizedSearchCV results saved to randomsearch_results_rq_matern.csv")
    ...
    cv_results_df.to_csv(config["data_dir"] / "cv_scores_random_rq_matern.csv", index=False)
    print("[INFO]: Cross-validation scores saved to cv_scores_random_rq_matern.csv")
    ...
    print("[INFO]: Predictions saved to prediction_random_rq_matern.csv")
