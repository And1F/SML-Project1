...
if __name__ == "__main__":
    ...
    matern_kernel = Matern(length_scale=1.0, nu=1.5, length_scale_bounds=(1e-5, 1e5))
    ...
    # Combined kernel
    combined_kernel = C(1.0, (1e-3, 1e3)) * (matern_kernel + rq_kernel)
    ...
    joblib.dump(random_search.best_estimator_, config["data_dir"] / "pipeline_random_matern_rq_white.joblib")
    print("[INFO]: Best pipeline saved to data/pipeline_random_matern_rq_white.joblib")

    results_df = pd.DataFrame(random_search.cv_results_)
    results_df.to_csv(config["data_dir"] / "randomsearch_results_matern_rq_white.csv", index=False)
    print("[INFO]: RandomizedSearchCV results saved to randomsearch_results_matern_rq_white.csv")
    ...
    cv_results_df.to_csv(config["data_dir"] / "cv_scores_random_matern_rq_white.csv", index=False)
    print("[INFO]: Cross-validation scores saved to cv_scores_random_matern_rq_white.csv")

    test_images = load_test_dataset(config)
    test_images = np.array(test_images)
    test_preds = random_search.predict(test_images)
    save_results(test_preds)
    print("[INFO]: Predictions saved to prediction_random_matern_rq_white.csv")
