...
if __name__ == "__main__":
    ...
    param_dist = {
        ...
        'regressor__kernel__k1__k1__constant_value': uniform(0.5, 5.0),
        'regressor__kernel__k1__k2__k1__length_scale': uniform(0.5, 5.0),
        'regressor__kernel__k1__k2__k2__length_scale': uniform(0.5, 5.0),
        'regressor__kernel__k1__k2__k2__alpha': uniform(0.5, 5.0),
        'regressor__kernel__k2__noise_level': uniform(1e-3, 1.0)
    }
    ...
    model_path = config["data_dir"] / f"pipeline_light_{timestamp}.joblib"
    joblib.dump(random_search.best_estimator_, model_path)

    results_df = pd.DataFrame(random_search.cv_results_)
    results_df.to_csv(config["data_dir"] / f"randomsearch_results_light_{timestamp}.csv", index=False)
    ...
    cv_results_df.to_csv(config["data_dir"] / f"cv_scores_light_{timestamp}.csv", index=False)
