...
if __name__ == "__main__":
    ...
    # Combine them
    combined_kernel = C(1.0, (1e-3, 1e3)) * (rq_kernel + rbf_kernel)

    # Define pipeline
    pipeline = Pipeline([
        ('scaler', StandardScaler()),
        ('pca', PCA(n_components=50)),
        ('regressor', GaussianProcessRegressor(kernel=combined_kernel, alpha=1e-2, random_state=42, normalize_y=True, ))
    ])
    ...
    param_dist = {
        ...
        'regressor__kernel__k2__k1__length_scale': uniform(0.1, 2.0),
        'regressor__kernel__k2__k1__alpha': uniform(0.1, 5.0),
        'regressor__kernel__k2__k2__length_scale': uniform(0.1, 2.0)
    }
    ...
    # Save best pipeline
    joblib.dump(random_search.best_estimator_, config["data_dir"] / "pipeline_random_rq_rbf.joblib")
    print("[INFO]: Best pipeline saved to data/pipeline_random_rq_rbf.joblib")

    # Save RandomizedSearchCV results to CSV
    results_df = pd.DataFrame(random_search.cv_results_)
    results_df.to_csv(config["data_dir"] / "randomsearch_results_rq_rbf.csv", index=False)
    print("[INFO]: RandomizedSearchCV results saved to randomsearch_results_rq_rbf.csv")
    ...
    cv_results_df.to_csv(config["data_dir"] / "cv_scores_random_rq_rbf.csv", index=False)
    print("[INFO]: Cross-validation scores saved to cv_scores_random_rq_rbf.csv")
    ...
    print("[INFO]: Predictions saved to prediction_random_rq_rbf.csv")
