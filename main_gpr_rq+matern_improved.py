...
from sklearn.gaussian_process.kernels import RationalQuadratic, RBF, ConstantKernel as C, WhiteKernel
...
if __name__ == "__main__":
    ...
    rbf_kernel = RBF(length_scale=1.0, length_scale_bounds=(1e-10, 1e10))
    white_kernel = WhiteKernel(noise_level=1.0, noise_level_bounds=(1e-5, 1e5))

    # Combine them
    combined_kernel = C(1.0, (1e-3, 1e3)) * (rq_kernel + rbf_kernel)  # Kernel without noise level
    full_kernel = combined_kernel + white_kernel  # Adding noise model separately
    ...
    # ConstantKernel length_scale
    'regressor__kernel__k1__k1__constant_value': uniform(0.1, 5.0),

    # RationalQuadratic (which is k2 in the inner sum, i.e. k1__k2)
    'regressor__kernel__k1__k2__k1__length_scale': uniform(0.1, 5.0),
    'regressor__kernel__k1__k2__k1__alpha': uniform(0.1, 10.0),

    # RBF (which is k2 in the sum, i.e. k1__k2__k2)
    ...
    # Save best pipeline
    joblib.dump(random_search.best_estimator_, config["data_dir"] / "pipeline_random_rq_rbf_white.joblib")
    print("[INFO]: Best pipeline saved to data/pipeline_random_rq_rbf_white.joblib")

    # Save RandomizedSearchCV results to CSV
    results_df = pd.DataFrame(random_search.cv_results_)
    results_df.to_csv(config["data_dir"] / "randomsearch_results_rq_rbf_white.csv", index=False)
    print("[INFO]: RandomizedSearchCV results saved to randomsearch_results_rq_rbf_white.csv")
    ...
    cv_results_df.to_csv(config["data_dir"] / "cv_scores_random_rq_rbf_white.csv", index=False)
    print("[INFO]: Cross-validation scores saved to cv_scores_random_rq_rbf_white.csv")
    ...
    print("[INFO]: Predictions saved to prediction_random_rq_rbf_white.csv")
